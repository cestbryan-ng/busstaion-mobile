...
export default function CreateAgency() {
  ...
  const validateStep2 = (): boolean => {
    const e: FormErrors = {};
    if (!form.longName.trim()) e.longName = t.required;
    else if (form.longName.trim().length < 3) e.longName = t.required;
  ...
  const Field = ({
    ...
    const errorKey = Object.keys(form).find(
      k => form[k as keyof FormData] === value,
...
const styles = StyleSheet.create({
  ...
  stepLabel: { ...typography.body, fontSize: 10, textAlign: 'center' },
  ...
  summaryLabel: { ...typography.bodyBold, fontSize: typography.sizes.md },
  confirmStationCard: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: spacing.md,
