...
export default function OrgVehicles() {
  ...
  const [filterSeats, setFilterSeats] = useState<'all' | 'small' | 'medium' | 'large'>('all');
  ...
  const handleCreateVehicle = async () => {
    if (!vNom.trim() || !vNbrPlaces.trim() || !vPlaque.trim()) {
      toast.warning(lang === 'fr' ? 'Champs obligatoires manquants' : 'Required fields missing');
      return;
    }
    setCreating(true);
    try {
      const res = await fetch(`${API_URL}/vehicule`, {
        ...
        body: JSON.stringify({
          ...
          lienPhoto: vPhoto.trim() || undefined,
      ...
      if (res.ok) {
        ...
        setVNom(''); setVModele(''); setVDescription(''); setVNbrPlaces(''); setVPlaque(''); setVPhoto('');
      } else {
        toast.error(lang === 'fr' ? 'Erreur lors de la création' : 'Creation failed');
      }
    } catch {
  ...
  const filteredVehicles = useMemo(() =>
    vehicles.filter(v => {
      const matchSearch = !debouncedSearch.trim() || [v.nom, v.modele, v.plaqueMatricule].some(f => f?.toLowerCase().includes(debouncedSearch.toLowerCase()));
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.backgroundAlt }]}>
      ...
      <View style={[styles.searchRow, { backgroundColor: theme.background, borderBottomColor: theme.border }]}>
        ...
        <TouchableOpacity
          style={[styles.filterBtn, { borderColor: showFilters ? colors.primary : theme.border, backgroundColor: showFilters ? `${colors.primary}15` : 'transparent' }]}
          onPress={() => setShowFilters(v => !v)}
        >
          <Ionicons name="options-outline" size={20} color={showFilters ? colors.primary : theme.textStrong} />
        </TouchableOpacity>
      </View>
      ...
      {showFilters && (
        <View style={[styles.filterChips, { backgroundColor: theme.background, borderBottomColor: theme.border }]}>
          {([
            ...
            <TouchableOpacity
              key={f.key}
              style={[styles.chip, { backgroundColor: filterSeats === f.key ? colors.primary : theme.backgroundAlt, borderColor: filterSeats === f.key ? colors.primary : theme.border }]}
              onPress={() => setFilterSeats(f.key)}
            >
              <Text style={[styles.chipText, { color: filterSeats === f.key ? '#fff' : theme.text }]}>{f.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
      ...
      >
        ...
        <View style={styles.classesSectionHeader}>
          <Text style={[styles.sectionTitle, { color: theme.textStrong }]}>{t.title}</Text>
        ...
        {filteredVehicles.length === 0 ? (
          <EmptyState type="result" message={t.noVehicles} textColor={theme.text} />
        ) : (
          filteredVehicles.map(v => (
            <View key={v.idVehicule} style={[styles.vehicleCard, { backgroundColor: theme.background, borderColor: theme.border }]}>
              <View style={[styles.vehicleImageBox, { backgroundColor: `${colors.primary}15` }]}>
                {v.lienPhoto && v.lienPhoto.startsWith('http') ? (
                  <Image source={{ uri: v.lienPhoto }} style={styles.vehicleImageFull} resizeMode="cover" />
                ) : (
              ...
              <View style={styles.vehicleInfo}>
                ...
                {v.modele && v.nom && (
                  <Text style={[styles.vehicleModel, { color: theme.text }]} numberOfLines={1}>{v.modele}</Text>
                )}
        ...
        {classes.length === 0 ? (
          <EmptyState type="result" message={t.noClasses} textColor={theme.text} />
        ) : (
          classes.map(cls => {
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { backgroundColor: theme.background }]}>
            ...
            <ScrollView contentContainerStyle={styles.modalBody} keyboardShouldPersistTaps="handled">
              {[
                { label: lang === 'fr' ? 'Nom *' : 'Name *', value: vNom, set: setVNom, placeholder: 'Toyota Coaster' },
                { label: lang === 'fr' ? 'Modèle' : 'Model', value: vModele, set: setVModele, placeholder: 'Coaster 2020' },
                { label: lang === 'fr' ? 'Plaque matricule *' : 'Plate number *', value: vPlaque, set: setVPlaque, placeholder: 'LT-1234-AB' },
                { label: lang === 'fr' ? 'Nb de places *' : 'Seats *', value: vNbrPlaces, set: setVNbrPlaces, placeholder: '30', keyboardType: 'numeric' as const },
                { label: lang === 'fr' ? 'Photo (URL)' : 'Photo (URL)', value: vPhoto, set: setVPhoto, placeholder: 'https://…' },
                { label: lang === 'fr' ? 'Description' : 'Description', value: vDescription, set: setVDescription, placeholder: '…', multiline: true },
              ].map(field => (
                <View key={field.label} style={styles.modalField}>
                  <Text style={[styles.modalLabel, { color: theme.text }]}>{field.label}</Text>
                  <TextInput
                    style={[styles.modalInput, { borderColor: theme.border, color: theme.textStrong, backgroundColor: theme.backgroundAlt, height: field.multiline ? 72 : 44, textAlignVertical: field.multiline ? 'top' : 'center' }]}
                    value={field.value}
            ...
            <View style={[styles.modalFooter, { borderTopColor: theme.border }]}>
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]} onPress={() => setShowVehicleModal(false)}>
                <Text style={[styles.modalBtnText, { color: theme.textStrong }]}>{lang === 'fr' ? 'Annuler' : 'Cancel'}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: colors.primary }]} onPress={handleCreateVehicle} disabled={creating}>
                {creating ? <ActivityIndicator size="small" color="#fff" /> : <Text style={[styles.modalBtnText, { color: '#fff' }]}>{lang === 'fr' ? 'Créer' : 'Create'}</Text>}
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={[styles.modalSheet, { backgroundColor: theme.background }]}>
            ...
            <View style={styles.modalBody}>
              <View style={styles.modalField}>
                <Text style={[styles.modalLabel, { color: theme.text }]}>{lang === 'fr' ? 'Nom *' : 'Name *'}</Text>
                <View style={styles.classChips}>
                  {['VIP', 'PREMIUM', 'STANDARD', 'ECONOMIQUE'].map(n => (
                    <TouchableOpacity key={n} style={[styles.classChip, { backgroundColor: cNom === n ? colors.primary : theme.backgroundAlt, borderColor: cNom === n ? colors.primary : theme.border }]} onPress={() => setCNom(n)}>
                      <Text style={[styles.classChipText, { color: cNom === n ? '#fff' : theme.text }]}>{n}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              <View style={styles.modalField}>
                <Text style={[styles.modalLabel, { color: theme.text }]}>{lang === 'fr' ? 'Prix (FCFA) *' : 'Price (FCFA) *'}</Text>
                <TextInput
                  style={[styles.modalInput, { borderColor: theme.border, color: theme.textStrong, backgroundColor: theme.backgroundAlt, height: 44 }]}
                  value={cPrix}
            ...
            <View style={[styles.modalFooter, { borderTopColor: theme.border }]}>
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: theme.backgroundAlt, borderColor: theme.border }]} onPress={() => setShowClassModal(false)}>
                <Text style={[styles.modalBtnText, { color: theme.textStrong }]}>{lang === 'fr' ? 'Annuler' : 'Cancel'}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modalBtn, { backgroundColor: colors.primary }]} onPress={handleCreateClass} disabled={creating}>
                {creating ? <ActivityIndicator size="small" color="#fff" /> : <Text style={[styles.modalBtnText, { color: '#fff' }]}>{lang === 'fr' ? 'Créer' : 'Create'}</Text>}
              </TouchableOpacity>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  chip: { borderWidth: 1, borderRadius: 4, paddingHorizontal: spacing.md, paddingVertical: spacing.xs },
  ...
  vehicleImage: { width: 90, height: 90, justifyContent: 'center', alignItems: 'center' },
  vehicleImageBox: { width: 56, height: 56, borderRadius: 4, justifyContent: 'center', alignItems: 'center', overflow: 'hidden', margin: spacing.md },
  ...
  vehicleModel: { ...typography.body, fontSize: typography.sizes.xs, marginTop: 1 },
  ...
  modalSheet: { borderTopLeftRadius: 16, borderTopRightRadius: 16, maxHeight: '90%' },
  ...
  modalInput: { borderWidth: 1, borderRadius: 4, paddingHorizontal: spacing.md, ...typography.body, fontSize: typography.sizes.sm },
  modalFooter: { flexDirection: 'row', gap: spacing.md, padding: spacing.lg, borderTopWidth: 1 },
  modalBtn: { flex: 1, height: 48, borderRadius: 4, borderWidth: 1, justifyContent: 'center', alignItems: 'center' },
  modalBtnText: { ...typography.bodyBold, fontSize: typography.sizes.md },
  classChips: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm },
  classChip: { borderWidth: 1, borderRadius: 4, paddingHorizontal: spacing.md, paddingVertical: spacing.sm },
